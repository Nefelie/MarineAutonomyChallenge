    ...
    def simulate(self):
        ...
        while not self._mission:
            ...
            if self._current_SIG is not None:
                ...
                current_SIG = float(self._current_SIG)
            else:
                past_SIG.append(self._current_SIG)
                current_SIG = self._current_SIG


            # save data to text file
            with open('pollutant_log.txt', 'a') as f:
                f.write(f'{current_pos_DEG[0]}, {current_pos_DEG[1]}, {current_SIG}\n')
            ...
            set_plot(waypoints=waypoints_list, current_pos=current_pos_DEG, current_speed=self._current_speed,
                     current_err=cross_t_err, pollutant_conc=self._current_SIG, limits=plot_limits, current_heading=self._current_heading,
                     path=track, axis=ax)
